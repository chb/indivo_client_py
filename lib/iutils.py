...
class IUtils(OAuth):
  ...
  def get_content_type(self, parameters, data):
    if not self.is_binary(data):
      if (isinstance(data, str) or isinstance(data, unicode)) and \
        len(data.strip()) > 0 and \
        data.strip()[0] == self.chars.lt:
    ...
    try:
      ...
      if xml_string[0:5].lower() != error and len(data_loc) > 1:
        for i in xrange(0, len(data_loc), 2):
          ...
          if xml_string is not None and \
            isinstance(xml_string, str):
            xmldoc = minidom.parseString(xml_string.strip())
            doc_elem = xmldoc.documentElement
            if doc_elem.nodeName == xml_list_node and \
                hasattr(doc_elem.getAttributeNode(xml_id_node), 'value') and \
                doc_elem.getAttributeNode(xml_id_node).value:
  ...
  def url_check(self, url):
    ...
    if not hasattr(self.url, 'url'):
      self.url.url = url
    if self.url and self.url.url and \
        self.url_scheme_check() and self.url.host:
  ...
  def url_scheme_check(self):
    ...
    if self.url.scheme == self.HTTP.http or \
        self.url.scheme == self.HTTP.https:
      return True
    else:
      return False
  ...
  def remove_unmatched_vars(self, path):
    ...
    count = 0
    for i in path:
      if i == self.chars.openbracket or \
          i == self.chars.closebracket:
        lst.append(count)
      count += 1
    lst.sort()
    if len(lst) % 2 == 0:
  ...
  def get_response(self, api_config_name, api_method, api_url, resp_data_loc, app_info, req_data=None, **kwargs):
    ...
    if kwargs.has_key(self.res.parameters) and \
        ...
        len(kwargs[self.res.parameters]) > 0:
